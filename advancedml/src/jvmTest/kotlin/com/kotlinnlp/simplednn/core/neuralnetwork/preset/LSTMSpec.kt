...
import com.kotlinnlp.simplednn.core.neuralnetwork.preset.LSTMNeuralNetwork
...
class LSTMSpec : Spek({
  ...
  describe("a LSTM Neural Netowork") {

    val hiddenActivation = ELU()
    val outputActivation = Softmax()
    val network = LSTMNeuralNetwork(
      inputSize = 3,
      hiddenSize = 5,
    ...
    context("initialization") {
      ...
      context("hidden layer configuration") {
        ...
        it("should a LSTM connection type") {
          assertEquals(LayerType.Connection.LSTM, hiddenLayerConfig.connectionType)
        }
      }
