  ...
  describe("A generic Layer") {
    ...
    context("initialization with dropout") {
      ...
      context("forward") {
        ...
        val zerosCount = (0 until inputArray.size).count { i -> inputArray.values[i] == 0.0f }

        it("should contain the expected number of dropped values") {
          // WARNING: This test is based on random generation of values
          assertTrue { equals(25000.0f, zerosCount.toFloat(), tolerance=500.0f) }
        }
        ...
        it("should contain the expected kept values") {
          assertTrue {
            (0 until inputArray.size).all { i ->
              ...
              value == 0.0f || equals(origValue / 0.75f, value, tolerance=1.0e-07f)
            }
          }
        }
      }
      ...
      context("backward") {
        ...
        val dropout = 0.40f
        val p: Float = 1.0f - dropout

        val origInput: DenseNDArray = DenseNDArrayFactory.arrayOf(floatArrayOf(-0.8f, -0.9f, -0.9f, 1.0f))
        ...
        it("should match the expected errors of the input array") {
          ...
          assertTrue {
            (0 until input.length).all { i ->
              inputErrors[i] == 0.0f || inputErrors[i] == outputErrors[i] / p
            }
          }
        }
      }
