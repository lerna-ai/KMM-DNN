...
class ActivableArraySpec : Spek({
  ...
  describe("an ActivableArray") {
    ...
    val initArray = DenseNDArrayFactory.arrayOf(floatArrayOf(0.0f, 0.1f, 0.01f, -0.1f, -0.01f, 1.0f, 10.0f, -1.0f, -10.0f))
    val initArrayWrongSize = DenseNDArrayFactory.arrayOf(floatArrayOf(0.0f, 0.1f, 0.01f, -0.1f))
    val activationFunction = ELU(alpha = 1.0f)
    val expectedActivatedValues = DenseNDArrayFactory.arrayOf(floatArrayOf(
      0.0f, 0.1f, 0.01f, -0.09516258f, -0.00995017f, 1.0f, 10.0f, -0.63212055f, -0.9999546f
    ))
    val activatedValuesDeriv = DenseNDArrayFactory.arrayOf(floatArrayOf(
      1.0f, 1.0f, 1.0f, 0.9048374f, 0.99004984f, 1.0f, 1.0f, 0.36787945f, 0.0000454f
    ))
    ...
    context("before activation") {
      ...
      it("should return the expected activated values calling getActivatedValues()") {
        assertTrue { outActivatedValues.equals(expectedActivatedValues, tolerance = 1.0e-08f) }
      }
    }
    ...
    context("after activation") {
      ...
      it("should have the expected activated values") {
        assertTrue { expectedActivatedValues.equals(activableArray.values, tolerance = 1.0e-08f) }
      }

      it("should have the expected values of the derivative of its activation") {
        assertTrue { activatedValuesDeriv.equals(activableArray.calculateActivationDeriv(), tolerance = 1.0e-07f) }
      }
      ...
      it("should return the same activated values calling getActivatedValues()") {
        assertTrue { outActivatedValues.equals(activableArray.values, tolerance = 1.0e-08f) }
      }
    }
    ...
    context("after activated twice") {
      ...
      val expectedActivatedValues2 = DenseNDArrayFactory.arrayOf(floatArrayOf(
        0.0f, 0.1f, 0.01f, -0.0907749f, -0.00990083f, 1.0f, 10.0f, -0.4685364f, -0.63210386f
      ))
      ...
      it("should have the expected activated values") {
        assertTrue { expectedActivatedValues2.equals(activableArray.values, tolerance = 1.0e-07f) }
      }
      ...
      it("should return the previous activated values as valuesNotActivated") {
        assertTrue { expectedActivatedValues.equals(activableArray.valuesNotActivated, tolerance = 1.0e-08f) }
      }
    }
