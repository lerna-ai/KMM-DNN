...
class FeedforwardNeuralProcessorSpec : Spek({
  ...
  describe("a FeedforwardNeuralProcessor") {
    ...
    context("forward") {
      ...
      val input: DenseNDArray = DenseNDArrayFactory.arrayOf(floatArrayOf(-0.8f, -0.9f, -0.9f, 1.0f))
      ...
      it("should match the expected output values") {
        ...
        assertTrue {
          processor.getOutput().equals(
            DenseNDArrayFactory.arrayOf(floatArrayOf(-0.862641f, -0.427188f, 0.183369f)),
            tolerance = 1.0e-06f)
        }
      }
    }
    ...
    context("backward") {
      ...
      val input: DenseNDArray = DenseNDArrayFactory.arrayOf(floatArrayOf(-0.8f, -0.9f, -0.9f, 1.0f))
      val outputErrors: DenseNDArray = DenseNDArrayFactory.arrayOf(floatArrayOf(-0.4f, -0.4f, -0.3f))
      ...
      it("should match the expected input errors") {
        ...
        assertTrue {
          processor.getInputErrors().equals(
            DenseNDArrayFactory.arrayOf(floatArrayOf(-0.811233f, -0.182174f, 0.417836f, -0.291757f)),
            tolerance = 1.0e-06f)
        }
      }
    }
