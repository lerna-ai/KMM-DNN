 ...
 *
 */
class SimpleRecurrentSpec : Spek({

  describe("a SimpleRecurrent Neural Netowork") {

    val hiddenActivation = ELU()
    val outputActivation = Softmax()
    val network = SimpleRecurrentNeuralNetwork(
      inputSize = 3,
      hiddenSize = 5,
    ...
    context("initialization") {
      ...
      context("hidden layer configuration") {
        ...
        it("should a Feedforward connection type") {
          assertEquals(LayerType.Connection.SimpleRecurrent, hiddenLayerConfig.connectionType)
        }
      }
