...
class DistributionArraySpec : Spek({
  ...
  describe("an DistributionArray") {
    ...
    context("factory") {
      ...
      context("uniform distribution") {
        ...
        it("should create an array with values uniformly distributed") {
          assertTrue { (0 until 10).all { i -> array.values[i] == 0.1f } }
        }
      }
      ...
      context("one hot distribution") {
        ...
        it("should contain the expected number of zeros") {
          val zerosCount = (0 until 10).count{ i -> array.values[i] == 0.0f }
          assertEquals(9, zerosCount)
        }

        it("should contain the expected 1.0 value") {
          assertEquals(1.0f, array.values[5])
        }
    ...
    context("initialization") {
      ...
      val initArray = DenseNDArrayFactory.arrayOf(floatArrayOf(0.0f, 0.5f, 0.2f, 0.1f, 0.2f))
      val wrongInitArray = DenseNDArrayFactory.arrayOf(floatArrayOf(0.0f, 0.6f, 0.2f, 0.1f, 0.2f))
      val wrongInitArrayNeg = DenseNDArrayFactory.arrayOf(floatArrayOf(0.0f, 0.5f, 0.2f, -0.1f, 0.2f))
    ...
    context("cloning") {
      ...
      it("should return an array with equal values") {
        assertTrue { array.values.equals(cloneArray.values, tolerance = 1.0e-08f) }
      }
    }
  }
})
